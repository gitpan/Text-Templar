...
#  $Id: Templar.pod,v 1.18 2003/01/15 21:20:19 deveiant Exp $
#
#  Authors: Michael Granger <ged@FaerieMUD.org>
#  and Dave McCorkhill <scotus@FaerieMUD.org>
#
#  Copyright (c) 1998-2003 Michael Granger and The FaerieMUD Consortium. All
#  rights reserved.
#
...
L<Class::Translucent|Class::Translucent>, L<IO::File|IO::File>,
L<IO::Handle|IO::Handle>, L<Scalar::Util|Scalar::Util>,
L<Data::Dumper|Data::Dumper>, L<Text::Templar::Parser|Text::Templar::Parser>,
L<Text::Templar::Exceptions|Text::Templar::Exceptions>
...
Copyright (c) 1998-2003 Michael Granger and The FaerieMUD Consortium. All rights
reserved.

This module is free software. You may use, modify, and/or redistribute this
software under the terms of the Perl Artistic License. (See
L<http://language.perl.com/misc/Artistic.html>)
...
the L<Text::Templar::Exceptions|Text::Templar::Exceptions> module.

=head3 Interaction Phase
...
The codeblock you specify will have access to any L<DEFINE|"DEFINE Directive">d
variables as lexical scalars, and will also have at least the following
constants defined:
...
telephone numbers, you could look up phone number associated with the 'work' key
...
Similarly, you can look up a single element from an arrayref by adding its index
to the methodchain:

  customer.orders.5.orderNumber

=head3 quotedArgument
...
L<"name"> attribute, and only exists inside of the loop, much the same way
...
Creates a template method with the specified L<"name">, which when called, will
insert its arguments into the rendered output at any place the named directive
appears. A method may appear with the same L<"name"> more than once, in which case
the arguments passed to the method appear in each place the method directive is
defined. Methods may have an optional L<"format"> or L<"codeblock"> attribute.

The L<"format"> attribute is a C<printf>-style format string which is applied to
each template value after it has been rendered.

The L<"codeblock"> attribute is a Perl subroutine which acts as a preprocessor for
...
=head3 METHODCALL directive

  <% METHODCALL <name> <methodchain> <format> %>
  <% METHODCALL <name> <methodchain> %>

Creates a template method with the specified L<"name">. When the template is
...
Methodcall directives may have an optional L<"format"> attribute.
...
The B<DEFINE> directive causes a template attribute with the L<"name"> specified to
be defined. These attributes will thenceforth be available to any evaluated
L<"codeblock"> in the same template.

The first directive attribute can either be a L<"name">, in which case it is
assumed to be a scalar value of the same L<"name">, or a L<"variable">.

The '=' part of the directive is syntactic sugar, and doesn't really cause
anything different to happen.

The second argument can be either a quoted string or a L<"codeblock"> to be
evaluated. The result from the L<"codeblock"> will be assigned to the defined
attribute.
...
=head3 STOP directive

  <% STOP %>

The B<STOP> directive indicates that the parser should stop rendering at the
point at which it is encountered. Template content below the B<STOP> directive
...
The B<CUT> directive is a top-wise STOP; it indicates that the parser should not
start rendering until the point at which it is encountered. Template content
above the B<CUT> directive will be parsed, and will be reflected in the
structure of the template object, but will not show up when rendered.
...
which it is encountered. The value inserted can be either a L<"variable"> that is
either L<"DEFINE">d elsewhere, or one of the constants defined by the
Text::Templar class itself (see the L<"codeblock"> for more
...
This directive inserts the environment variable specified by L<"name"> into the
rendered template output after being formatted by the optional L<"format"> string.
...
=head3 INHERIT directive

  <% INHERIT <name> %>

Inherit the content container with the specified L<"name"> from an enclosing
...
The C<FOREACH> directive is an L<"iterator"> directive for inserting a
bit of content once for each item of a list.

The first form calls the specified L<"methodchain"> on the L<"name">d data,
loops over each element of the return value, and inserts the contents contained
within the opening and closing tags once for each iteration with the L<"iterator">
set to the current item. The 'C<IN>' is just syntactic sugar, and may be
omitted.

The second form does the same thing, but the L<"name">d parameter is assumed to be
...
cause the L<"name">d data to be dereferenced first. This is sometimes useful with
nested FOREACH directives, where the outer loop may be iterating over an array
of arrayrefs, for example.

The fifth through twelfth forms are hash L<"iterator">s. They expect a hash or
hashref as the value of L<"name">, or as the return value of the specified
L<"methodchain"> called on the L<"name">d object. It iterates over each key/value
pair, setting L<"iterator"> to a hashref with C<key> and C<value> pairs. The forms
that include C<SORT> or C<SORTED> (either may be used interchangeably), sort by the
given criteria; C<BY_KEY> causes the iteration to be sorted (stringwise) by each
pair's C<key> element; C<BY_VALUE> does the same for each pair's C<value>
element. The forms that specify a L<"codeblock"> act like the L<"codeblock"> of a
L<"SORT">

The fifth form iterates over the L<"name">d list, using the L<"name"> as the
...
The C<JOIN> directive acts like a L<FOREACH|"FOREACH Directive"> L<"iterator">
that joins the rendered output of each of its loops together with the string
specified by the L<"quotedArgument">.

The operation of the join directive is otherwise the same as that for the
L<FOREACH|"FOREACH Directive"> directive.
...
The first form calls the specified L<"methodchain"> on the L<"name">d object and
uses the L<"codeblock"> to sort the list. The two elements to be compared are
...
array of values are assigned to the temporary L<"iterator">, and the contents
between the opening and closing C<SORT> tags is inserted once into the rendered
output.

The second form sorts in the same manner, but without a L<"methodchain"> call on the
L<"name">d parameter.

The third form substitutes the sorted list for the specified L<"name"> inside the
contained content instead of using an L<"iterator">.
...
The first form calls the specified L<"methodchain"> on the L<"name">d object and
executes the L<"codeblock"> for each element. It assigns the array of return
values of the L<"codeblock"> calls to the temporary L<"iterator">, and then inserts
the contents contained between the opening and closing tags once.

The second form transforms the L<"name">d content with the specified L<"codeblock">
...
=head3 GREP directive

  <% GREP <iterator> [IN] <name> <methodchain> [WITH] <codeblock> %>
  <% GREP <name> [WITH] <codeblock> %>

The C<GREP> directive is used to modify a content array using L<"codeblock"> as a
filter function.

The first form calls the specified L<"methodchain"> on the L<"name">d object and
executes the L<"codeblock"> for each element. If the L<"codeblock"> returns a true
...
is added to the temporary L<"iterator"> array, and the contents of the tag are
inserted once.

The second form filters the L<"name">d content with the specified L<"codeblock">
...
specified in one of five ways: the result of a L<"methodchain">, a literal integer,
the value passed to a method call, the result of a code block, or the value of a
specified L<"variable">.
...
content of the specified L<"name">, the specified L<"methodchain"> of the
last-inserted L<"object">, or the specified L<"codeblock"> or L<"variable"> in its
...
content of the specified L<"name">, the specified L<"methodchain"> of the last-inserted
L<"object">, or the specified L<"codeblock"> or L<"variable"> match the
L<"matchspec">. If the L<"matchspec"> is a regexp, each item must match it, if the
L<"matchspec"> is an array or a hash, each item must be equal (stringwise) to at
least one of the items in the array or one of the keys of the hash.

The words in brackets are syntactic sugar, and may be omitted.

=head3 ELSE directive
...
same rules as the L<IF|"IF Directive"> directive.
...
Get/set the attribute which turns syntax tree caching on or off. If this is set
to a true value, the template object's syntax tree will be reused the next time
it is loaded, skipping the parse phase altogether. (Translucent method)
...
=item I<_buildHashIteratedContent( $node, @content )>

Attempt to build a list of hashrefs with key and value pairs for hash
iterators.

=item I<_buildIteratedContent( $node )>
...
Evaluate the L<"matchspec"> associated with the given node with the node's
...
=item I<_shiftTailedErrors( undef )>
...
Traverse each element in a L<"methodchain">, calling a method or fetching a value
